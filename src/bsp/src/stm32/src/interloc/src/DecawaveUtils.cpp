...
bool DecawaveUtils::isFrameResponse(const UWBRxFrame& rxFrame) {
    return reinterpret_cast<const UWBMessages::DWFrame*>(rxFrame.m_rxBuffer.data())
               ->m_functionCode == UWBMessages::FunctionCode::TWR_RESPONSE;
}

bool DecawaveUtils::isFrameFinal(const UWBRxFrame& rxFrame) {
    return reinterpret_cast<const UWBMessages::DWFrame*>(rxFrame.m_rxBuffer.data())
               ->m_functionCode == UWBMessages::FunctionCode::TWR_FINAL;
}

bool DecawaveUtils::isFramePoll(const UWBRxFrame& rxFrame) {
    return reinterpret_cast<const UWBMessages::DWFrame*>(rxFrame.m_rxBuffer.data())
               ->m_functionCode == UWBMessages::FunctionCode::TWR_POLL;
}