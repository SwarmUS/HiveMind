 ...
 * */
__INLINE uint32_t portGetTickCnt(void) { return HAL_GetTick(); }
 ...
 * */
__INLINE void Sleep(uint32_t x) { HAL_Delay(x); }
 ...
 * */
__INLINE uint32_t port_CheckEXT_IRQ(void) { return HAL_GPIO_ReadPin(DECAIRQ_GPIO, DW_IRQn_Pin); }
 ...
 *
 * NOTE:
 *   - As EXTI9_5_IRQHandler does not check that port_deca_isr is not null, the user application
 * must ensure that a proper handler is set by calling this function before any DW1000 IRQ occurs!
