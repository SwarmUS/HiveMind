...
#define DWT_START_RX_DELAYED                                                                       \
    1 // Set up delayed RX, if "late" error triggers, then the RX will be enabled immediately
#define DWT_IDLE_ON_DLY_ERR                                                                        \
    2 // If delayed RX failed due to "late" error then if this
      // flag is set the RX will not be re-enabled immediately, and device will be in IDLE when
      // function exits
#define DWT_NO_SYNC_PTRS                                                                           \
    4 // Do not try to sync IC side and Host side buffer pointers when enabling RX. This is used to
      // perform manual RX re-enabling when receiving a frame in double buffer mode.
...
#define DWT_FF_COORD_EN                                                                            \
    0x002 // behave as coordinator (can receive frames with no dest address (PAN ID has to match))
...
#define DWT_CONFIG                                                                                 \
    0x0040 // ONW_LDC - on wakeup restore (load) the saved configurations (from AON array into HIF)
#define DWT_LOADEUI 0x0008 // ONW_LEUI - on wakeup load EUI
#define DWT_RX_EN 0x0002 // ONW_RX - on wakeup activate reception
#define DWT_TANDV                                                                                  \
    0x0001 // ONW_RADC - on wakeup run ADC to sample temperature and voltage sensor values
...
#define DWT_DW_WUP_NO_UCODE                                                                        \
    0x04 // init after wake up - ucode has not already been loaded / ucode is not used
...
// GPIOs
#define DWT_GxP0                                                                                   \
    0x00000001UL /* GPIO0 Only changed if the GxM0 mask bit has a value of 1 for the write         \
                    operation*/
...
typedef struct {
    ...
    uint8 phrMode; //!< PHR mode {0x0 - standard DWT_PHRMODE_STD, 0x3 - extended frames
                   //!< DWT_PHRMODE_EXT}
...
/*
 * From version 5.0.0:
 *  - dwt_getinitxtaltrim: Replaced by the function dwt_getxtaltrim which returns current xtal trim
 * value
 *
 * From version 4.0.0:
 ...
 *  - dwt_setrxmode: Replaced by dwt_setsniffmode and dwt_setlowpowerlistening depending on the RX
 * mode the user wants to set up.
 *  - dwt_checkoverrun: As automatic RX re-enabling is not supported anymore, this functions has
 * become useless.
 *  - dwt_setautorxreenable: As automatic RX re-enabling is not supported anymore, this functions
 * has become useless.
 *  - dwt_getrangebias: Range bias correction values are platform dependent and should therefore be
 * managed at user application level.
 *  - dwt_xtaltrim: Renamed to dwt_setxtaltrim.
 *  - dwt_checkIRQ: Renamed to dwt_checkirq.
 *
 * From version 3.0.0:
 *  - dwt_getldotune: As LDO loading is now automatically managed by the driver, this function has
 * become useless.
 *  - dwt_getotptxpower: TX power values and location in OTP memory are platform dependent and
 * should therefore be managed at user application level.
 *  - dwt_readantennadelay: Antenna delay values and location in OTP memory are platform dependent
 * and should therefore be managed at user application level.
 ...
 *
 * @brief This is used to senable GPIO clocks. The clocks are needed to ensure correct GPIO
 * operation
 *
 * input parameters
 *
 ...
 * level abstract functions to write to and read from the SPI the definitions should be in
 * deca_spi.c file. Low level abstract function to write to the SPI Takes two separate byte buffers
 * for write header and write data returns 0 for success, or -1 for error
 *
 ...
 * @param headerBuffer  - pointer to buffer containing the 'headerLength' bytes of header to be
 * written
 ...
 * @param readBuffer    - pointer to buffer containing to return the data (NB: size required =
 * headerLength + readlength)
 *
 * output parameters
 *
 * returns DWT_SUCCESS for success (and the position in the buffer at which data begins), or
 * DWT_ERROR for error
 */
...
// ---------------------------------------------------------------------------
//
// NB: The purpose of the deca_mutex.c file is to provide for microprocessor interrupt
...
//
//     In porting this to a particular microprocessor, the implementer may choose to use #defines
//     here to map these calls transparently to the target system.  Alternatively the appropriate
//     code may be embedded in the functions provided in the deca_irq.c file.
//
// ---------------------------------------------------------------------------
 ...
 * @fn decamutexoff()
 *
 * @brief This function should re-enable interrupts, or at least restore their state as
