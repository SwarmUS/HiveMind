...
// Defines for ACK request bitmask in DATA and MAC COMMAND frame control (first byte) - Used to
// detect AAT bit wrongly set.
 ...
 *
 * @brief This function sets the local data structure pointer to point to the element in the local
 * array as given by the index.
 *
 * input parameters
 * @param index    - selects the array element to point to. Must be within the array bounds, i.e. <
 * DWT_NUM_DW_DEV
 *
 * output parameters
 *
 ...
 * function can be called to initialise internal structure  dw1000local[] if it has not been
 * preserved (e.g. if micro was in sleep and its RAM data (containing dw1000local structure was not
 ...
 *                  DWT_LOADUCODE        0x01 - load the LDE microcode from ROM - enable accurate RX
 * timestamp DWT_DW_WAKE_UP       0x02 - just initialise dw1000local[] values (e.g. DW1000 has woken
 * up) DWT_DW_WUP_NO_UCODE  0x04 - if microcode/LDE algorithm has not already been loaded (on power
 * up) e.g. when LDE is not used DWT_READ_OTP_PID     0x10 - read part ID from OTP DWT_READ_OTP_LID
 * 0x20 - read lot ID from OTP DWT_READ_OTP_BAT     0x40 - read ref voltage from OTP
...
int dwt_initialise(int config) {
    ...
    if (!(DWT_DW_WAKE_UP & config)) // Don't reset the device if DWT_DW_WAKE_UP bit is set, e.g.
                                    // when calling this API after wake up
    {
        dwt_softreset(); // Make sure the device is completely reset before starting initialisation
    }

    if (!((DWT_DW_WAKE_UP & config) && ((DWT_READ_OTP_TMP | DWT_READ_OTP_BAT | DWT_READ_OTP_LID |
                                         DWT_READ_OTP_PID | DWT_DW_WUP_RD_OTPREV) &
                                        config))) {
        _dwt_enableclocks(FORCE_SYS_XTI); // NOTE: set system clock to XTI - this is necessary to
    ...
    // When DW1000 IC is initialised from power up, then the LDO value should be kicked from OTP,
    // otherwise if this API is called after DW1000 IC has been woken up (DWT_DW_WAKE_UP bit is set)
    // this can be skipped as LDO would have already been automatically kicked/loaded on wake up
    ...
    } else { // if LDOTUNE reg contains value different from default it means it was kicked from OTP
             // and thus set AON_WCFG_ONW_LLDO.
        if (dwt_read32bitoffsetreg(RF_CONF_ID, LDOTUNE) != LDOTUNE_DEFAULT)
            pdw1000local->sleep_mode |= AON_WCFG_ONW_LLDO;
    }

    if ((!(DWT_DW_WAKE_UP & config)) ||
        ((DWT_DW_WAKE_UP & config) && (DWT_DW_WUP_RD_OTPREV & config))) {
        ...
        pdw1000local->otprev =
            0; // If OTP valuse are not used, if this API is called after DW1000 IC has been woken
               // up (DWT_DW_WAKE_UP bit is set), set otprev to 0

    if (!(DWT_DW_WAKE_UP & config)) {
        // XTAL trim value is set in OTP for DW1000 module and EVK/TREK boards but that might not be
        // the case in a custom design
        if ((otp_xtaltrim_and_rev & 0x1F) ==
            0) // A value of 0 means that the crystal has not been trimmed
        {
            otp_xtaltrim_and_rev =
    ...
    if (!(DWT_DW_WAKE_UP & config)) {
        if (DWT_LOADUCODE & config) {
            ...
            pdw1000local->sleep_mode |= AON_WCFG_ONW_LLDE; // microcode must be loaded at wake-up if
    ...
    } else // if DWT_DW_WUP_NO_UCODE is set then assume that the UCODE was loaded from ROM (i.e.
           // DWT_LOADUCODE was set on power up),
    { // thus set AON_WCFG_ONW_LLDE, otherwise don't set the AON_WCFG_ONW_LLDE bit in the sleep_mode
      // configuration
    ...
    // The 3 bits in AON CFG1 register must be cleared to ensure proper operation of the DW1000 in
    // DEEPSLEEP mode.
    dwt_write8bitoffsetreg(AON_ID, AON_CFG1_OFFSET, 0x00);

    // Read system register / store local copy
    pdw1000local->sysCFGreg = dwt_read32bitreg(SYS_CFG_ID); // Read sysconfig register
    pdw1000local->longFrames = (pdw1000local->sysCFGreg & SYS_CFG_PHR_MODE_11) >>
                               SYS_CFG_PHR_MODE_SHFT; // configure longFrames
 ...
 *
 * NOTE: dwt_initialise() must be called prior to this function so that it can return a relevant
 * value.
 *
 * input parameters
 *
 ...
 * @fn dwt_setlnapamode()
 *
 * @brief This is used to enable GPIO for external LNA or PA functionality - HW dependent, consult
 * the DW1000 User Manual. This can also be used for debug as enabling TX and RX GPIOs is quite
 * handy to monitor DW1000's activity.
 *
 * NOTE: Enabling PA functionality requires that fine grain TX sequencing is deactivated. This can
 * be done using dwt_setfinegraintxseq().
 *
 * input parameters
 ...
 * @fn dwt_setgpiovalue()
 *
 * @brief This is used to set GPIO value as (1) or (0) only applies if the GPIO is configured as
 * output
 *
 * input parameters
 ...
 * @fn dwt_getgpiovalue()
 *
 * @brief This is used to return 1 or 0 depending if the depending if the GPIO is high or low, only
 * one GPIO should be tested at a time
 *
 * input parameters
 * @param gpioNum    -   this is the GPIO to configure - see DWT_GxP0... DWT_GxP8
 *
 ...
 * @fn dwt_geticrefvolt()
 *
 * @brief This is used to return the read V measured @ 3.3 V value recorded in OTP address 0x8
 * (VBAT_ADDRESS)
 *
 * NOTE: dwt_initialise() must be called prior to this function so that it can return a relevant
 * value.
 ...
 * @fn dwt_geticreftemp()
 *
 * @brief This is used to return the read T measured @ 23 C value recorded in OTP address 0x9
 * (VTEMP_ADDRESS)
 *
 * NOTE: dwt_initialise() must be called prior to this function so that it can return a relevant
 * value.
 ...
 * @fn dwt_readdevid()
 *
 * @brief This is used to return the read device type and revision information of the DW1000 device
 * (MP part is 0xDECA0130)
 *
 * input parameters
 *
 * output parameters
 ...
 *
 * input parameters
 * @param config    -   pointer to the txrf configuration structure, which contains the tx rf config
 * data
 *
 * output parameters
 *
 ...
 *    These modifications optimise the default OPS configuration further for 64 length preamble use
 * case
 *
 * NOTE: These register settings are not preserved during SLEEP/DEEPSLEEP, thus they should be
 * programmed again after wake up
 *
 * input parameters
 * @param prf
 *
 ...
 *
 * input parameters
 * @param config    -   pointer to the configuration structure, which contains the device
 * configuration data.
 *
 * output parameters
 *
...
void dwt_configure(dwt_config_t* config) {
    ...
    pdw1000local->sysCFGreg |=
        (SYS_CFG_PHR_MODE_11 & ((uint32)config->phrMode << SYS_CFG_PHR_MODE_SHFT));
    ...
    regval = (CHAN_CTRL_TX_CHAN_MASK & (chan << CHAN_CTRL_TX_CHAN_SHIFT)) | // Transmit Channel
             ...
             (CHAN_CTRL_TX_PCOD_MASK &
              ((uint32)config->txCode << CHAN_CTRL_TX_PCOD_SHIFT)) | // TX Preamble Code
             (CHAN_CTRL_RX_PCOD_MASK &
              ((uint32)config->rxCode << CHAN_CTRL_RX_PCOD_SHIFT)); // RX Preamble Code

    dwt_write32bitreg(CHAN_CTRL_ID, regval);

    // Set up TX Preamble Size, PRF and Data Rate
    pdw1000local->txFCTRL =
        ((uint32)(config->txPreambLength | config->prf) << TX_FCTRL_TXPRF_SHFT) |
        ((uint32)config->dataRate << TX_FCTRL_TXBR_SHFT);
    dwt_write32bitreg(TX_FCTRL_ID, pdw1000local->txFCTRL);

    // The SFD transmit pattern is initialised by the DW1000 upon a user TX request, but (due to an
    // IC issue) it is not done for an auto-ACK TX. The SYS_CTRL write below works around this
    // issue, by simultaneously initiating and aborting a transmission, which correctly initialises
    // the SFD after its configuration or reconfiguration. This issue is not documented at the time
    // of writing this code. It should be in next release of DW1000 User Manual (v2.09, from July
    // 2016).
 ...
 * @fn dwt_writetxfctrl()
 *
 * @brief This API function configures the TX frame control register before the transmission of a
 * frame
 *
 * input parameters:
...
void dwt_writetxfctrl(uint16 txFrameLength, uint16 txBufferOffset, int ranging) {
    ...
    assert((ranging == 0) || (ranging == 1))
        ...
        uint32 reg32 = pdw1000local->txFCTRL | txFrameLength |
                       ((uint32)txBufferOffset << TX_FCTRL_TXBOFFS_SHFT) |
 ...
 * @fn dwt_readrxdata()
 *
 * @brief This is used to read the data from the RX buffer, from an offset location give by offset
 * parameter
 *
 * input parameters
 ...
 * @fn dwt_readaccdata()
 *
 * @brief This is used to read the data from the Accumulator buffer, from an offset location give by
 * offset parameter
 *
 * NOTE: Because of an internal memory access delay when reading the accumulator the first octet
 * output is a dummy octet that should be discarded. This is true no matter what sub-index the read
 * begins at.
 *
 * input parameters
 * @param buffer - the buffer into which the data will be read
 ...
 *
 * @brief This is used to read the RX carrier integrator value (relating to the frequency offset of
 * the TX node)
 *
 * NOTE: This is a 21-bit signed quantity, the function sign extends the most significant bit, which
 * is bit #20 (numbering from bit zero) to return a 32-bit signed integer value.
 *
 * input parameters - NONE
 *
 * return value - the (int32) signed carrier integrator value.
 *                A positive value means the local RX clock is running faster than the remote TX
 * device.
 */
 ...
 *
 * input parameters
 * @param diagnostics - diagnostic structure pointer, this will contain the diagnostic data read
 * from the DW1000
 *
 * output parameters
 *
 ...
 *
 * @brief This is used to read the high 32-bits of the TX timestamp (adjusted with the programmed
 * antenna delay)
 *
 * input parameters
 *
 ...
 *
 * @brief This is used to read the low 32-bits of the TX timestamp (adjusted with the programmed
 * antenna delay)
 *
 * input parameters
 *
 ...
 *
 * @brief This is used to read the high 32-bits of the RX timestamp (adjusted with the programmed
 * antenna delay)
 *
 * input parameters
 *
 ...
 *
 * @brief This is used to read the low 32-bits of the RX timestamp (adjusted with the programmed
 * antenna delay)
 *
 * input parameters
 *
 ...
 *        a. check if sub index is used, if subindexing is used - set bit-6 to 1 to signify that the
 * sub-index address follows the register index byte b. set bit-7 (or with 0x80) for write operation
 *        c. if extended sub address index is used (i.e. if index > 127) set bit-7 of the first
 * sub-index byte following the first header byte
 *
...
void dwt_writetodevice(uint16 recordNumber, uint16 index, uint32 length, const uint8* buffer) {
    ...
    // Write message header selecting WRITE operation and addresses as appropriate (this is one to
    // three bytes long)
    if (index == 0) // For index of 0, no sub-index is required
    {
        header[cnt++] = 0x80 | recordNumber; // Bit-7 is WRITE operation, bit-6 zero=NO
...
#endif
        header[cnt++] = 0xC0 | recordNumber; // Bit-7 is WRITE operation, bit-6 one=sub-address
        ...
        if (index <= 127) // For non-zero index < 127, just a single sub-index byte is required
        {
            ...
            header[cnt++] =
                0x80 |
                (uint8)(
                    index); // Bit-7 one means extended index, bits 6-0 is low seven bits of index.
...
void dwt_readfromdevice(uint16 recordNumber, uint16 index, uint32 length, uint8* buffer) {
    ...
    // Write message header selecting READ operation and addresses as appropriate (this is one to
    // three bytes long)
    if (index == 0) // For index of 0, no sub-index is required
    {
        header[cnt++] = (uint8)recordNumber; // Bit-7 zero is READ operation, bit-6 zero=NO
                                             // sub-addressing, bits 5-0 is reg file id
    } else {
#ifdef DWT_API_ERROR_CHECK
        assert(
...
#endif
        header[cnt++] =
            (uint8)(0x40 | recordNumber); // Bit-7 zero is READ operation, bit-6 one=sub-address
                                          // follows, bits 5-0 is reg file id
 ...
 *      DWT_FF_COORD_EN         0x002   behave as coordinator (can receive frames with no
 * destination address (PAN ID has to match)) DWT_FF_BEACON_EN        0x004   beacon frames allowed
 ...
 * @param length - this is the number of 32 bit words to read (array needs to be at least this
 * length)
 *
 * output parameters
 *
...
void dwt_otpread(uint16 address, uint32* array, uint8 length) {
    ...
    _dwt_enableclocks(FORCE_SYS_XTI); // NOTE: Set system clock to XTAL - this is necessary to make
...
int dwt_otpwriteandverify(uint32 value, uint16 address) {
    ...
    // For each value to program - the readback/check is done couple of times to verify it has
    ...
    // Even if the above does not exit before retry reaches 10, the programming has probably been
    // successful
 ...
 *
 * @brief This function uploads always on (AON) configuration, as set in the AON_CFG0_OFFSET
 * register.
 *
 * input parameters
 *
 ...
 *
 * @brief This function uploads always on (AON) data array and configuration. Thus if this function
 * is used, then _dwt_aonconfigupload is not necessary. The DW1000 will go so SLEEP straight after
 * this if the DWT_SLP_EN has been set.
 *
 * input parameters
 *
 * output parameters
 ...
 * @fn dwt_entersleep()
 *
 * @brief This function puts the device into deep sleep or sleep. dwt_configuresleep() should be
 * called first to configure the sleep and on-wake/wake-up parameters
 *
 * input parameters
 *
 * output parameters
 ...
 *
 * @brief sets the sleep counter to new value, this function programs the high 16-bits of the 28-bit
 * counter
 *
 * NOTE: this function needs to be run before dwt_configuresleep, also the SPI frequency has to be <
 * 3MHz
 *
 * input parameters
 ...
 *
 * @brief calibrates the local oscillator as its frequency can vary between 7 and 13kHz depending on
 * temp and voltage
 *
 * NOTE: this function needs to be run before dwt_configuresleepcnt, so that we know what the
 * counter units are
 *
 * input parameters
 *
 * output parameters
 *
 * returns the number of XTAL/2 cycles per low-power oscillator cycle. LP OSC frequency = 19.2
 * MHz/return value
 */
 ...
 * i.e. before entering the sleep, the device should be programmed for TX or RX, then upon "waking
 * up" the TX/RX settings will be preserved and the device can immediately perform the desired
 * action TX/RX
 *
 * NOTE: e.g. Tag operation - after deep sleep, the device needs to just load the TX buffer and send
 * the frame
 *
 *
 ...
 *
 * input parameters
 * @param enable - 1 to configure the device to enter deep sleep after TX, 0 - disables the
 * configuration
 *
 * output parameters
 *
 ...
 *
 * input parameters
 * @param buff   - this is a pointer to the dummy buffer which will be used in the SPI read
 * transaction used for the WAKE UP of the device
 ...
 *                  DWT_OPSET_64LEN = 0x0 - load the operational parameter set table for 64 length
 * preamble configuration DWT_OPSET_TIGHT = 0x1 - load the operational parameter set table for tight
 * xtal offsets (<1ppm) DWT_OPSET_DEFLT = 0x2 - load the default operational parameter set table
 * (this is loaded from reset)
 *
 * output parameters
 *
 * no return value
 ...
 *
 * @brief This call enables the auto-ACK feature. If the responseDelayTime (parameter) is 0, the ACK
 * will be sent a.s.a.p. otherwise it will be sent with a programmed delay (in symbols), max is 255.
...
void dwt_setrxaftertxdelay(uint32 rxDelayTime) {
    ...
    val |=
        (rxDelayTime &
         ACK_RESP_T_W4R_TIM_MASK); // In UWB microseconds (e.g. turn the receiver on 20uus after TX)
 ...
 * @fn dwt_setcallbacks()
 *
 * @brief This function is used to register the different callbacks called when one of the
 * corresponding event occurs.
 *
 * NOTE: Callbacks can be undefined (set to NULL). In this case, dwt_isr() will process the event as
 * usual but the 'null' callback will not be called.
 *
 * input parameters
 ...
 * @fn dwt_isr()
 *
 * @brief This is the DW1000's general Interrupt Service Routine. It will process/report the
 following events:
 ...
 *        For all events, corresponding interrupts are cleared and necessary resets are performed.
 In addition, in the RXFCG case,
 *        received frame information and frame control are read before calling the callback. If
 double buffering is activated, it
 *        will also toggle between reception buffers once the reception callback processing has
 ended.
 *
 *        /!\ This version of the ISR supports double buffering but does not support automatic RX
 re-enabling!
 *
 * NOTE:  In PC based system using (Cheetah or ARM) USB to SPI converter there can be no interrupts,
 however we still need something
 *        to take the place of it and operate in a polled way. In an embedded system this function
 should be configured to be triggered
...
void dwt_isr(void) {
    ...
    if (status & SYS_STATUS_RXFCG) {
        ...
        // Report frame length - Standard frame length up to 127, extended frame length up to 1023
        // bytes
        ...
        // Because of a previous frame not being received properly, AAT bit can be set upon the
        // proper reception of a frame not requesting for acknowledgement (ACK frame is not actually
        // sent though). If the AAT bit is set, check ACK request bit in frame control to confirm
        // (this implementation works only for IEEE802.15.4-2011 compliant frames). This issue is
        // not documented at the time of writing this code. It should be in next release of DW1000
        // User Manual (v2.09, from July 2016).
        if ((status & SYS_STATUS_AAT) &&
            ((pdw1000local->cbData.fctrl[0] & FCTRL_ACK_REQ_MASK) == 0)) {
            dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_AAT); // Clear AAT status bit in register
            pdw1000local->cbData.status &=
                ~SYS_STATUS_AAT; // Clear AAT status bit in callback data register copy
    ...
    if (status & SYS_STATUS_TXFRS) {
        ...
        // In the case where this TXFRS interrupt is due to the automatic transmission of an ACK
        // solicited by a response (with ACK request bit set) that we receive through using
        // wait4resp to a previous TX (and assuming that the IRQ processing of that TX has already
        // been handled), then we need to handle the IC issue which turns on the RX again in this
        // situation (i.e. because it is wrongly applying the wait4resp after the ACK TX). See
        // section "Transmit and automatically wait for response" in DW1000 User Manual
    ...
    if (status & SYS_STATUS_ALL_RX_TO) {
        ...
        // Because of an issue with receiver restart after error conditions, an RX reset must be
        // applied after any error or timeout event to ensure the next good frame's timestamp is
        // computed correctly. See section "RX Message timestamp" in DW1000 User Manual.
        dwt_forcetrxoff();
        dwt_rxreset();
 ...
 * @fn dwt_isr_lplisten()
 *
 * @brief This is the DW1000's Interrupt Service Routine to use when low-power listening scheme is
 * implemented. It will only process/report the RXFCG event (through cbRxOk callback). It clears
 * RXFCG interrupt and reads received frame information and frame control before calling the
 * callback.
 *
...
void dwt_lowpowerlistenisr(void) {
    ...
    // The only interrupt handled when in low-power listening mode is RX good frame so proceed
    // directly to the handling of the received frame.

    // Deactivate low-power listening before clearing the interrupt. If not, the DW1000 will go back
    // to sleep as soon as the interrupt is cleared.
    ...
    // Because of a previous frame not being received properly, AAT bit can be set upon the proper
    // reception of a frame not requesting for acknowledgement (ACK frame is not actually sent
    // though). If the AAT bit is set, check ACK request bit in frame control to confirm (this
    // implementation works only for IEEE802.15.4-2011 compliant frames).
    // This issue is not documented at the time of writing this code. It should be in next release
    // of DW1000 User Manual (v2.09, from July 2016).
 ...
 *
 * @brief This function disables the TX blocks sequencing, it disables PMSC control of RF blocks,
 * system clock is also set to XTAL
 *
 * input parameters none
 *
 * output parameters none
 ...
 * @fn dwt_starttx()
 *
 * @brief This call initiates the transmission, input parameter indicates which TX mode is used see
 * below
 *
 * input parameters:
 ...
 *               if mode = DWT_START_TX_IMMEDIATE | DWT_RESPONSE_EXPECTED - immediate TX (response
 * expected - so the receiver will be automatically turned on after TX is done) if mode =
 * DWT_START_TX_DELAYED | DWT_RESPONSE_EXPECTED - delayed TX (response expected - so the receiver
 * will be automatically turned on after TX is done)
 *
 * output parameters
 *
 * returns DWT_SUCCESS for success, or DWT_ERROR for error (e.g. a delayed transmission will be
 * cancelled if the delayed time has passed)
 */
...
int dwt_starttx(uint8 mode) {
    ...
    if (mode & DWT_START_TX_DELAYED) {
        ...
        if ((checkTxOK & SYS_STATUS_TXERR) ==
            0) // Transmit Delayed Send set over Half a Period away or Power Up error (there is
               // enough time to send but not to power up individual blocks).
        {
            retval = DWT_SUCCESS; // All okay
        } else {
            // If HPDWARN or TXPUTE are set this indicates that the TXDLYS was set too late for the
            // specified DX_TIME. remedial action is to cancel delayed send and report error
...
void dwt_syncrxbufptrs(void) {
    ...
    {
        dwt_write8bitoffsetreg(
            SYS_CTRL_ID, SYS_CTRL_HRBT_OFFSET,
 ...
 *
 * SNIFF mode is a low-power reception mode where the receiver is sequenced on and off instead of
 * being on all the time. The time spent in each state (on/off) is specified through the parameters
 * below. See DW1000 User Manual section 4.5 "Low-Power SNIFF mode" for more details.
 *
 * input parameters:
 * @param enable - 1 to enable SNIFF mode, 0 to disable. When 0, all other parameters are not taken
 * into account.
 * @param timeOn - duration of receiver ON phase, expressed in multiples of PAC size. The counter
 * automatically adds 1 PAC size to the value set. Min value that can be set is 1 (i.e. an ON time
 * of 2 PAC size), max value is 15.
 * @param timeOff - duration of receiver OFF phase, expressed in multiples of 128/125 �s (~1 �s).
 * Max value is 255.
 *
 * output parameters
 *
 ...
 * periodically, (after this "long sleep"), for a very short time to sample the air for a preamble
 * sequence. This preamble sampling "listening" phase is actually two reception phases separated by
 * a "short sleep" time. See DW1000 User Manual section "Low-Power Listening" for more details.
 *
 * NOTE: Before enabling low-power listening, the following functions have to be called to fully
 * configure it:
 *           - dwt_configuresleep() to configure long sleep phase. "mode" parameter should at least
 * have DWT_PRESRV_SLEEP, DWT_CONFIG and DWT_RX_EN set and "wake" parameter should at least have
 * both DWT_WAKE_SLPCNT and DWT_SLP_EN set.
 ...
 *       When configured, low-power listening mode can be triggered either by putting the DW1000 to
 * sleep (using dwt_entersleep()) or by activating reception (using dwt_rxenable()).
 *
 *       Please refer to the low-power listening examples (examples 8a/8b accompanying the API
 * distribution on Decawave's website). They form a working example code that shows how to use
 ...
 *
 * input parameters:
 * @param snooze_time - "short sleep" phase duration, expressed in multiples of 512/19.2 �s (~26.7
 * �s). The counter automatically adds 1 to the value set. The smallest working value that should be
 * set is 1, i.e. giving a snooze time of 2 units (or ~53 �s).
 *
 * output parameters
 *
 * no return value
 ...
 * DWT_START_RX_DELAYED        1 used to set up delayed RX, if "late" error triggers, then the RX
 * will be enabled immediately (DWT_START_RX_DELAYED | DWT_IDLE_ON_DLY_ERR) 3 used to disable
 * re-enabling of receiver if delayed RX failed due to "late" error (DWT_START_RX_IMMEDIATE |
 * DWT_NO_SYNC_PTRS) 4 used to re-enable RX without trying to sync IC and host side buffer pointers,
 * typically when performing manual RX re-enabling in double buffering mode
 *
 * returns DWT_SUCCESS for success, or DWT_ERROR for error (e.g. a delayed receive enable will be
 * too far in the future if delayed time has passed)
 */
...
void dwt_setrxtimeout(uint16 time) {
    ...
    if (time > 0) {
        ...
        dwt_write8bitoffsetreg(SYS_CFG_ID, 3,
                               temp); // Write at offset 3 to write the upper byte only
    } else {
        temp &=
            ~((uint8)(SYS_CFG_RXWTOE >> 24)); // Shift RXWTOE mask as we read the upper byte only
 ...
 *
 * input parameters
 * @param  timeout - Preamble detection timeout, expressed in multiples of PAC size. The counter
 * automatically adds 1 PAC size to the value set. Min value that can be set is 1 (i.e. a timeout of
 * 2 PAC size).
 *
 ...
 * @param operation - if set to 1 the interrupts (only the ones selected in the bitmask) are enabled
 * else they are cleared
 *                  - if set to 2 the interrupts in the bitmask are forced to selected state - i.e.
 * the mask is written to the register directly.
 *
 * output parameters
 *
 * no return value
...
void dwt_readeventcounters(dwt_deviceentcnts_t* counters) {
    ...
    temp =
        dwt_read32bitoffsetreg(DIG_DIAG_ID, EVC_PHE_OFFSET); // Read sync loss (31-16), PHE (15-0)
    ...
    temp =
        dwt_read32bitoffsetreg(DIG_DIAG_ID, EVC_FWTO_OFFSET); // Read RX TO (31-16), TXFRAME (15-0)
...
void dwt_softreset(void) {
    ...
    // DW1000 needs a 10us sleep to let clk PLL lock after reset - the PLL will automatically lock
    // after the reset Could also have polled the PLL lock flag, but then the SPI needs to be < 3MHz
    // !! So a simple delay is easier
    deca_sleep(1);
 ...
 * @fn dwt_getxtaltrim()
 *
 * @brief This function returns current value of XTAL trim. If this is called after dwt_initalise it
 * will return the OTP value if OTP value is non-zero or FS_XTALT_MIDRANGE if OTP value is zero (not
 * programmed).
 *
 * input parameters
 *
 ...
 *
 * @brief this function sets the DW1000 to continuous tx frame mode for regulatory approvals
 * testing.
 *
 * input parameters:
 * @param framerepetitionrate - This is a 32-bit value that is used to set the interval between
 * transmissions. The minimum value is 4. The units are approximately 8 ns. (or more precisely
...
void dwt_configcontinuousframemode(uint32 framerepetitionrate) {
    ...
    dwt_write8bitoffsetreg(DIG_DIAG_ID, DIAG_TMC_OFFSET,
                           (uint8)(DIAG_TMC_TX_PSTM)); // Turn the tx power spectrum test mode -
...
uint16 dwt_readtempvbat(uint8 fastSPI) {
    ...
    {
        _dwt_enableclocks(FORCE_SYS_XTI); // NOTE: set system clock to XTI - this is necessary to
                                          // make sure the values read are reliable
        // Reading All SAR inputs
        wr_buf[0] = 0x00;
        dwt_writetodevice(TX_CAL_ID, TC_SARL_SAR_C, 1, wr_buf);
        wr_buf[0] = 0x01; // Set SAR enable
...
float dwt_convertrawtemperature(uint8 raw_temp) {
    ...
    assert(pdw1000local->otp_mask & DWT_READ_OTP_TMP);
#endif
    // the User Manual formula is: Temperature (�C) = ( (SAR_LTEMP � OTP_READ(Vtemp @ 23�C) )
    // x 1.14) + 23
 ...
 * and applies the conversion factor to give a value in IC temperature units, as produced by the SAR
 * A/D. The dwt_initialise needs to be called before call to this to ensure pdw1000local->tempP
 * contains the SAR_LTEMP value from OTP.
 *
 * input parameters:
 * @param externaltemp - this is the an externally measured temperature in 10ths of degrees Celcius
 * to convert
 *
 * output parameters:
 *
...
float dwt_convertrawvoltage(uint8 raw_voltage) {
    ...
    assert(pdw1000local->otp_mask & DWT_READ_OTP_BAT);
#endif
    // the User Manual formula is: Voltage (V) = ( (SAR_LVBAT � OTP_READ(Vmeas @ 3.3 V) ) / 173 )
    // + 3.3
 ...
 * NOTE 2: The sleep to allow the calibration to complete is set to 1ms here, but can be as low as
 * 10us.
 *
 * input parameters:
...
uint32 dwt_calcbandwidthtempadj(uint16 target_count) {
    ...
    dwt_write8bitoffsetreg(PMSC_ID, PMSC_CTRL0_OFFSET,
                           PMSC_CTRL0_SYSCLKS_125M | PMSC_CTRL0_TXCLKS_125M);

    // Set the MSB high for first guess
    curr_bw = 0x80;
    // Set starting bit
    bit_field = 0x80;
    // Initial lowest delta is the maximum difference that we should allow the count value to be
    // from the target. If the algorithm is successful, it will be overwritten by a smaller value
    // where the count value is closer to the target
    delta_lowest = 300;
    ...
    for (i = 0; i < 7; i++) {
        ...
        dwt_write8bitoffsetreg(TX_CAL_ID, TC_PGCCTRL_OFFSET,
                               TC_PGCCTRL_DIR_CONV | TC_PGCCTRL_TMEAS_MASK);
        ...
        raw_count =
            dwt_read16bitoffsetreg(TX_CAL_ID, TC_PGCAL_STATUS_OFFSET) & TC_PGCAL_STATUS_DELAY_MASK;
 ...
 *
 * input parameters:
 * @param ref_powerreg - uint32 - the TX_POWER register value recorded when reference measurements
 * were made
    ...
    for (i = 0; i < 4; i++) {
        ...
        while ((current_mixer_gain + mixer_gain_change < 4) ||
            ...
            if (current_mixer_gain + mixer_gain_change > 20) {
                ...
                {
                    da_attn_change = 1; // restore the value and exit the loop - DA is at max
                                        // allowed
                    break;
                }
                ...
                {
                    da_attn_change = 7; // restore the value and exit the loop - DA is at min
                                        // allowed
                    break;
                }
 ...
 *
 * Note: only ch2 or ch5 are supported, if other channel is used - the COMP factor should be
 ...
 *                                  and the temperature at which reference measurements were made
 (raw value units)
 ...
 * @fn dwt_calcpgcount()
 *
 * @brief this function calculates the value in the pulse generator counter register (PGC_STATUS)
 * for a given PG_DELAY This is used to take a reference measurement, and the value recorded as the
 * reference is used to adjust the bandwidth of the device when the temperature changes.
 *
 * NOTE 1: SPI Frequency must be < 3MHz.
 * NOTE 2: The sleep to allow the calibration to complete is set to 1ms here, but can be as low as
 * 10us.
 *
 * input parameters:
 * @param pgdly - uint8 - the PG_DELAY to set (to control bandwidth), and to find the corresponding
 * count value for output parameters: None
 *
 * returns: (uint16) PGC_STATUS count value calculated from the provided PG_DELAY value - used as
 * reference for later bandwidth adjustments
 */
